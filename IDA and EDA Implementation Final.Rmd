...
## Data Inputation
One common method of handling missing values is to replace the missing values with the average of the relevant feature. For example, we can use the ave() function from the stats package or a custom function to calculate the average value and fill in the missing values accordingly. Since our dataset did not have any missing values. This step was not necessary.
...
## Data Reshaping
Functions like `pivot_wider()` and `pivot_longer()` from the tidyr package, along with mutate(), `filter()`, and `select()` from `dplyr`, allow us to effectively alter the structure of the dataset. Our dataset did not need to be reshaped.
...
This scatter plot shows the relationship between Population and Energy Consumption. From the plot, there is no discernible patter. The data points are scattered without a clear trend. Hence, linear regression might not be a good fit for modelling this relationship.
...
```{r}
scatter_log_energy <- ggplot(carbon_emissions, aes(x = Energy_Consumption_TWh, y = Population_Millions)) +
  geom_point(color = "blue", alpha = 0.6) +
